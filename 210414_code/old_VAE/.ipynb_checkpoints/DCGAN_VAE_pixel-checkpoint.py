...
class Encoder(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        main.add_module('input-relu', nn.ReLU(inplace=True))
        
        for i in range(n-3):
            # state size. (ngf) x 32 x 32
            main.add_module(f'pyramid:{ngf * 2 ** i}-{ngf * 2 ** (i+1)}:conv',
                            nn.Conv2d(ngf * 2 ** i, ngf * 2 ** (i+1), 4, 2, 1, bias=False))
            main.add_module(f'pyramid:{ngf * 2 ** (i+1)}:batchnorm',
                            nn.BatchNorm2d(ngf * 2 ** (i+1)))
            main.add_module(f'pyramid:{ngf * 2 ** (i+1)}:relu',
