...
#############################################################
    ...
    ans = input('Q. Which dataset do you want to train the model for??\n(1) CIFAR-10 (2) FMNIST (Press 1 or 2)\n')
    ...
    if ans == '1':
        ...
        print('Oops! Please insert 1 or 2. Bye~')
...
#############################################################
    ...
    for epoch in range(opt.niter):
        ...
        if epoch % 50 == 0 and epoch > 0:
            torch.save(netG.state_dict(), experiment + f'/netG_pixel_nz_{nz}_ngf_{ngf}_epoch_{epoch}.pth')
            torch.save(netE.state_dict(), experiment + f'/netE_pixel_nz_{nz}_ngf_{ngf}_epoch_{epoch}.pth')
            
    torch.save(netG.state_dict(), experiment + f'/netG_pixel_nz_{nz}_ngf_{ngf}_epoch_{opt.niter}.pth')
    torch.save(netE.state_dict(), experiment + f'/netE_pixel_nz_{nz}_ngf_{ngf}_epoch_{opt.niter}.pth')
