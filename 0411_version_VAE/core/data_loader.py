...
def TEST_loader(train_dist='cifar10', target_dist='cifar10', batch_size=1, shuffle=True, is_glow=False):
    ...
    if train_dist == 'cifar10':
        ...
        if target_dist == 'cifar10':
            return test_loader_cifar10(opt, preprocess, batch_size, shuffle)

        elif target_dist == 'svhn':
            return test_loader_svhn(opt, preprocess, batch_size, shuffle)

        elif target_dist == 'celeba':
            return test_loader_celeba(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'lsun':
            return test_loader_lsun(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'cifar100':
            return test_loader_cifar100(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'mnist':
            return test_loader_mnist(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'fmnist':
            return test_loader_fmnist(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'kmnist':
            return test_loader_kmnist(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'omniglot':
            return test_loader_omniglot(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'notmnist':
            return test_loader_notmnist(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'trafficsign':
            return test_loader_trafficsign(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'noise':
            return test_loader_noise(opt, preprocess, batch_size, shuffle)
        
        elif target_dist == 'constant':
            return test_loader_constant(opt, preprocess, batch_size, shuffle)
...
def test_loader_celeba(opt, preprocess, batch_size, shuffle):
    ...
    class CelebA(data.Dataset):
        def __init__(self, db_path = opt.dataroot+'/celeba/archive/', transform=None):
    ...
    celeba = CelebA(transform=transform)
    test_loader_celeba_ = data.DataLoader(
        celeba,
        batch_size=batch_size,
        shuffle=shuffle,
        num_workers=0,
    )
    return test_loader_celeba_

def test_loader_lsun(opt, preprocess, batch_size, shuffle):
    if opt.nc == 1:
        preprocess += [rgb_to_gray]
    class LSUN(data.Dataset):
        def __init__(self, db_path = opt.dataroot+'/LSUN_test', categories=['bedroom', 'kitchen'], transform=None):
...
def test_loader_notmnist(opt, preprocess, batch_size, shuffle):
    ...
    notmnist = notMNIST(opt.dataroot+'/notMNIST_small/', transform=transform)
...
def test_loader_trafficsign(opt, preprocess, batch_size, shuffle):
    ...
    ts = trafficsign(opt.dataroot+'/GTSRB_Final_Test_Images/Final_Test/Images', transform=transform)
