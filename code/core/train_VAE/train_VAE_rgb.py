...
#############################################################
    ...
    for epoch in range(opt.niter):
        ...
        for i, (x, _) in enumerate(dataloader):
            ...
            #recon = recon.view(-1, 3)
            #print(recon.shape)
            recl = loss_fn(recon, x) / b 
        ...
        if (epoch + 1) % 200== 0:
            torch.save(netG.state_dict(), experiment + f'/netG_rgb_ngf_{ngf}_nz_{nz}_beta_{beta}_augment_{opt.augment}_epoch_{epoch}.pth')
            torch.save(netE.state_dict(), experiment + f'/netE_rgb_ngf_{ngf}_nz_{nz}_beta_{beta}_augment_{opt.augment}_epoch_{epoch}.pth')
            
            
    torch.save(netG.state_dict(), experiment + f'/netG_rgb_ngf_{ngf}_nz_{nz}_beta_{beta}_augment_{opt.augment}_complete.pth')
    torch.save(netE.state_dict(), experiment + f'/netE_rgb_ngf_{ngf}_nz_{nz}_beta_{beta}_augment_{opt.augment}_complete.pth')
