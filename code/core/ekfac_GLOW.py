...
class EKFACOptimizer(optim.Optimizer):
    def __init__(self,
                 ...
                 damping=0.001,
                 batch_averaged=True):
        defaults = dict(lr=0, momentum=0, damping=damping,
                        weight_decay=0)
        ...
        #self.known_modules = {'Linear', 'Conv2d', 'InvertibleConv1x1'}
        # Note : This GLOW model has another 'Conv2d' class,
        # different with the standard Conv2d class contained in 'torch.nn'.
        ...
        if len(select_modules) != 0:
            ...
            assert type(self.select_modules) == list
        else:
            self.select_modules = []
    ...
    def _prepare_model(self):
        ...
        for name, module in self.model.named_modules():
            ...
            classname = str(module.__class__).split(' ')[1].split("'")[1]
            
            if len(self.select_modules) != 0:
                is_name_valid = name in self.select_modules
            else:
                is_name_valid = True
                
            if classname in self.known_modules and is_name_valid:
    ...
    def step(self, closure=None):
        ...
        self.stat_decay = 1 - 1 / (self.steps + 1)
        self.steps += 1
